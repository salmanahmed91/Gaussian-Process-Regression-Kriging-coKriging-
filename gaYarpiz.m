...
function [OptimalValue, fval] =  gaYarpiz(fun,nvar,varmin,varmax,MaxIt,nPop,intCon)
...
VarMin= varmin;         % Lower Bound of Variables
VarMax= varmax;         % Upper Bound of Variables
...
for i=1:nPop
    ...
    pop(i).Position(1,intCon) = round(pop(i).Position(1,intCon)); 
    % Evaluation
...
for it=1:MaxIt
    ...
    for k=1:nc/2
        ...
        % Apply Crossover
        [popc(k,1).Position, popc(k,2).Position]=Crossover(p1.Position,p2.Position,gamma,VarMin,VarMax,intCon);
    ...
    for k=1:nm
        ...
        % Apply Mutation
        popm(k).Position=Mutate(p.Position,mu,VarMin,VarMax,intCon);
    ...
    disp(['Itr ' num2str(it) ': Best Cost = ' num2str(BestCost(it)) ' BestPos = ' num2str(BestSol.Position)]);
...
OptimalValue = BestSol(1).Position;
fval         = BestCost;
